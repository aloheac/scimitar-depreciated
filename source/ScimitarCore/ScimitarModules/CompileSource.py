...
class CompileSource( PreExecutionModule ):
	...
	def getScriptContribution( self ):
		...
		if self.run.runSettings.optionCompileSource:
			...
			contribution += "if not os.path.isfile( self.run.runSettings.sourcePath + "/" + self.run.runSettings.executableFilename ):\n"
			contribution += "	print '>> ERROR: The source did not seem to compile successfully. Check for errors and try again.'"
